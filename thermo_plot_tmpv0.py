...
def vertically_integrate(value, alts, calc3D = False):
    [nLons, nLats, nAlts] = np.squeeze(value).shape
...
def read_in_model_files(args, header):
    ...
    if args.winds:
        if (ivar > 28):
            ...
            plot_vars.append(header['vars'].index('Ve') if args.cut in ['alt', 'lat']
                            else header['vars'].index('Vn'))
            plot_vars.append(header['vars'].index('Vv') if args.cut in ['lat', 'lon']
                             else header['vars'].index('Vn'))
    ...
    for j, filename in enumerate(header['filename']):
        ...
        if header['IsGitm']:
            ...
            if (header["HasHeader"]):
                ...
                data = read_routines.read_aether_file(filename, None)
                ivar = args.var
        for k, v in data.items():
            if isinstance(v, np.ndarray):
                data[k] = np.squeeze(v)
                if args.on2:
                    ivar = data['vars'].index('O')
        # For the first file, initialize the necessary plotting data
        if j == 0:
            # Get 1D arrays for the coordinates
            if header['GITMNC']:
                alts = np.sort(np.unique(data[data['vars'].index('Altitude')]))/1000.0
                lats = np.unique(sorted(data[data['vars'].index('lat')]))
                lons = np.unique(sorted(data[data['vars'].index('lon')]))
                lat2d, lon2d = np.meshgrid(lats, lons)
                Alts3d = np.zeros([len(lons), len(lats), len(alts)])
            ...
            if (args.cut == 'alt'):
                ...
                dlon = lons[1] - lons[0]#data[0][1, 0, 0] - data[0][0, 0, 0]
                dlat = lats[1] - lats[0]#data[1][0, 1, 0] - data[1][0, 0, 0]
        ...
        if (args.cut == 'alt'):
            #print(cut_data, ivar)
            #print(data[ivar].shape, args.cut, ivar)
            int_data = np.squeeze(data[ivar])[cut_data] * area
            if (args.mean):
        ...
        if args.tec:
            ...
            if (args.on2):
                ...
                #print(data['vars'])
                oDensity = data[data['vars'].index('O')]
                n2Density = data[data['vars'].index('N2')]
...
def get_file_info(args):
    ...
    if ((IsGitm) and (not HasHeader)):
        ...
        if (HasHeader):
            ...
            header = read_nc_header(args.filelist)
...
def read_nc_header(files):
    from netCDF4 import Dataset
    header = {"vars": [], "time": [], "filename": np.asarray(files)}
    for file in files:
        with Dataset(file, 'r') as ds:
            header['vars'] = list(ds.variables)
            #seconds since ...
            t_sec = ds['time'][0]
            t0 = datetime(*[int(i) for i in ds['time'].units.split('since ')[-1].split('-')])

            header['time'].append(timedelta(seconds=int(t_sec)) + t0)
